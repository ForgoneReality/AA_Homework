...
def my_rotate!(array, amt)
    #optimization exists in which we can do subsets to not do rotations many times over and over
    #or just use extra space like i do
    amt = amt % array.length
    bob = array.dup
    (0...array.length).each do |i|
        array[i] = bob[(i+amt)%array.length]
    end
    return array
end
